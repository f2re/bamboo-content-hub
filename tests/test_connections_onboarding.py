...
def test_connections_page_is_step_by_step_and_has_direct_provider_links(client):
    page = client.get("/connections")
    assert page.status_code == 200
    assert "Подключение за 2–3 шага" in page.text
    assert "Настройка без .env" in page.text
    ...
    assert "Создать приложение VK ID" in page.text
...
def test_oauth_credentials_can_be_saved_in_encrypted_provider_config():
    ...
    with SessionLocal() as db:
        account = merge_provider_config(
            ...
            settings,
            "google",
            {"oauth_client_id": "ui-client", "oauth_client_secret": "ui-secret"},
        )
